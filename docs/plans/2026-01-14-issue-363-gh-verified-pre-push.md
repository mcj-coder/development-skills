...
**Goal:** Allow the pre-push hook to treat GitHub-verified commits as acceptable when local GPG verification fails.

**Architecture:** Extend `.husky/pre-push` to query the GitHub API via `gh` for commits with invalid local signature status, and only block commits that remain unverified.
...
- [ ] `.husky/pre-push` checks GitHub verification when local signature status is invalid
- [ ] `.husky/pre-push` allows commits with `verification.verified == true` to pass
- [ ] `.husky/pre-push` still blocks commits that are unsigned or invalid and not GitHub verified
- [ ] `docs/playbooks/enable-signed-commits.md` documents GitHub-verified commit acceptance in the pre-push hook
...
- Use `gh api repos/{owner}/{repo}/commits/{sha}` and check `.commit.verification.verified`.
- Treat `true` as acceptable, otherwise fail.

**Step 3: Guard for `gh` availability**

- If `gh` is unavailable or repo cannot be resolved, keep existing failure behavior.
...
- Note that the pre-push hook accepts commits verified by GitHub even if local keys are missing.
...
- Run: `gh api repos/mcj-coder/development-skills/commits/<sha> --jq '.commit.verification'`
