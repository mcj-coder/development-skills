...
Establish clear TDD/BDD verification requirements distinguishing between concrete changes
(requiring applied evidence) and process-only skills (allowing analytical verification).
This baseline practice will unblock many other issues requiring clear verification standards.
...
**BDD Requirement:** Must demonstrate skill was applied in THIS repository.
Example: "TDD skill applied: failing test committed at [SHA], implementation at [SHA],
passing test at [SHA]"
...
**BDD Requirement:** Analytical verification acceptable. Example: "Brainstorming skill applied:
requirements clarified in issue comment [link], alternatives considered [link],
decision documented [link]"
